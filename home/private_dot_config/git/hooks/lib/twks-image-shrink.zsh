...
Example:
  ${0} -s ./photos -d ./output -w 1920 -q 80 -m 500K
EOF
}
...
main() {
  ...
  if ! [[ $width =~ '^[1-9][0-9]*$' ]]; then
    _zlog error "错误: 宽度必须为正整数，当前值: $width"
    return 1
  fi
  if ! [[ $quality =~ '^([1-9][0-9]?|100)$' ]]; then
    _zlog error "错误: 质量必须为 1-100，当前值: $quality"
    return 1
  fi
  if ! [[ $effort =~ '^[0-9]$' ]]; then
    _zlog error "错误: effort必须为 0-9，当前值: $effort"
    return 1
  fi
  ...
  _zlog info "共发现 ${total_found} 张图片。"

  local -a missing_files files_to_process
  local -i skip_count=0

  # 1. 优先过滤已存在且比源文件“Older”的目标文件 (成本最低)
